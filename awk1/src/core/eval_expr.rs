...
fn eval_fieldreference(reference: &AWKExpr, env: &mut AWKEnv) -> AWKVal {
    // -1などは整数に変換される
    // これは意図した動作ではない
...
fn eval_assign(lval: &AWKLval, expr: &AWKExpr, env: &mut AWKEnv) -> AWKVal {
...
fn eval_unary_operation(expr: &AWKExpr, op: &AWKUnaryOperation, env: &mut AWKEnv) -> AWKVal {
    let val = eval_awkexpr(expr, env);
    match op {
