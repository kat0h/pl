        ...
        (case (car exp)
          ...
          ((if)
           (if (true? (myeval (cadr exp) env))
               (self (caddr exp) env) ; tail call
               (if (null? (cdddr exp))
                   #f   ; else節がない場合の値
                   (self (cadddr exp) env)))) ; tailcall
          ((define) 
          ...
          (else
           ...
           (let ((func (myeval (car exp) env)) (arguments (map (lambda (x) (myeval x env)) (cdr exp))))
            (case (car func)
