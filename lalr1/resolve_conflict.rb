...
def get_precedence(g, a)
  return unless p = g.precedence.index { it[1].include?(a) }
...
def resolve_conflict_by_precedence(g, i, a, act, ca_i, debug = false)
  ...
  throw :reduce_reduce_conflict if act_reduce.size > 1
  ...
  reduceprec = i.map do |t|
    if t.dot < 2
      nil
    else
      t.r[t.dot - 2]
    end
  end.compact.uniq.map { get_precedence(g, it) }.max
